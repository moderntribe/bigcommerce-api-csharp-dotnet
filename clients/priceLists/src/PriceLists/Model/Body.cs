...
namespace PriceLists.Model
{
        ...
        public partial class Body :  IEquatable<Body>, IValidatableObject
    {
        ...
        public Body(int? customerGroupId = default(int?), int? priceListId = default(int?), int? channelId = default(int?))
        {
            // to ensure "priceListId" is required (not null)
            if (priceListId == null)
            {
                throw new InvalidDataException("priceListId is a required property for Body and cannot be null");
            }
            else
            {
